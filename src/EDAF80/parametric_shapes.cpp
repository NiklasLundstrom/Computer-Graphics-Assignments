...
parametric_shapes::createQuad(unsigned int width, unsigned int height)
{
	...
	glVertexAttribPointer(static_cast<unsigned int>(bonobo::shader_bindings::normals), 3, GL_FLOAT, GL_FALSE, 0, reinterpret_cast<GLvoid const*>(0x0));
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizeiptr>(indices.size() * sizeof(glm::uvec3)),indices.data(),GL_STATIC_DRAW);
                               ...
                               float const rB)
{
	...
	float theta = 0.0f,                                                           // 'stepping'-variable for small circle
		dtheta = glm::two_pi<float>() / (static_cast<float>(res_theta) - 1.0f); // step size, depending on the resolution

	float phi = 0.0f,                                                                     // 'stepping'-variable for big circle
		dphi = glm::two_pi<float>() / (static_cast<float>(res_phi) - 1.0f); // step size, depending on the resolution
	...
	for (unsigned int i = 0u; i < res_phi; i++){
		...
		for (unsigned int j = 0u; j < res_theta; j++){
			float cos_phi = std::cos(phi),
				sin_phi = std::sin(phi);
			// vertex
			vertices[index] = glm::vec3((rA + rB*std::cos(theta))*std::cos(phi),
										(rA + rB*std::cos(theta))*std::sin(phi),
										-rB*std::sin(theta));
			...
			// tangent
			auto t = glm::vec3(-rB*std::sin(theta)*std::cos(phi),
								-rB * std::sin(theta)*std::sin(phi),
								-rB*std::cos(theta));
			t = glm::normalize(t);
			tangents[index] = t;

			// binormal
			auto b = glm::vec3(-(rA + rB * std::cos(theta))*std::sin(phi),
								(rA + rB * std::cos(theta))*std::cos(phi),
								0);
			b = glm::normalize(b);
			binormals[index] = b;
